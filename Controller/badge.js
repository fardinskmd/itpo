...
if(!paymentDetails.amount || !paymentDetails.customerId || !paymentDetails.customerEmail || !paymentDetails.customerPhone) {
    ...
    var paytmChecksum = checksum_lib.generateSignature(params, config.PaytmConfig.key);
      ...
      try {
        ...
        badge.find({ORDERID:req.body.ORDERID}).then(data=>{
          ...
          for(let item of data){        
            sendMail(item.email,item.name,item.price,item.bookingDate,item.urnno,item.citizentype)
...
exports.updatePaymentStatus=async(req,res)=>{
  let status=''
  await paymentsfindOne({ORDERID:req.body.ORDERID}).then(data=>{
...
exports.addbadge = async (req, res) => {
 let urnno = 9+Math.random().toString(16).slice(50) + randomBytes(1).toString('hex') + Date.now()
    const { name,email,mobile, status,ticketid,price,date,citizentype,ORDERID,bookingDate} = req.body;
...
if(ticketid=="636b010d90351e553fa1afa7"){
  ...
  const bypassstatus="False"
    const tickets = new badge({name,email,mobile, status,ticketid,urnno,price,date,citizentype,bypassstatus,ORDERID,bookingDate});
    const newbadge= await tickets.save();
    ...
    if (newbadge) {
      ...
      res.status(200).json({
        Message: "Badge Added Succsesfully", tickets
      });
    }
...
exports.getByNumber=async(req,res)=>{
  ...
  badge.find({mobile:req.params.id}).then(data=>{
    console.log(data)
    res.status(200).json({
      resultData:data
      });
  ...
  console.log(req.params._id)
    badge.findOne({ sNo : req.params._id}).populate('ticketid') 
    .then(result=>{
        res.status(200).json({
        resultData:result
        });
  ...
  exports.getbadgebyurnnumber = async (req, res) => {
    console.log(req.params.urnno)
      badge.findOne({ urnno : req.params.urnno}).populate('ticketid') 
      .then(result=>{
          res.status(200).json({
          resultData:result
          });
  ...
  let date=req.body.date
      ...
      price.find({ id :Id,citizentype:req.body.citizentype,date:req.body.date})
      .then(result=>{
          res.status(200).json({
          resultData:result
          });
      ...
      sendMail=async (email,name,price,date,urnno,citizentype)=>{
        console.log('hello')
...
var optionalObj = {'fileName':  ImageFileName, 'type':'png'};
    ...
    exports.updatebadgestatus = async (req, res) => {
      try {
        ...
        const updatetype = await badge.findByIdAndUpdate(req.body._id, {
          status,userid,updateat
        })
        const updatelivestock = await updatetype.save();
        if (updatelivestock) {
          return res.status(200).json({ msg: "livestock type update successfulley", updatelivestock })
        }
        return res.status(200).json({ msg: "livestock type update successfulley", updatelivestock})
      } catch (error) {
        console.log(error)
  ...
  exports.gettotalticket = async (req, res) => {
    try {
        ...
        await badge.find({paymentStatus:"Paid"}).count().then(userData => {
...
exports.gettotalscaneeduser = async (req, res) => {
  try {
      ...
      await badge.find({status:"Marked"}).count().then(userData => {
          res.status(200).json({Data:userData})
          console.log(userData)
      })
...
exports.gettotalscaneeduserdatainadmin = async (req, res) => {
  try {
      ...
      await badge.find({paymentStatus:"Paid"}).populate('ticketid')
      .then(userData => {
          res.status(200).json({Data:userData})
          console.log(userData)
      })
...
exports.gettotalscaneeduserdata = async (req, res) => {
  try {
      ...
      await badge.find({status:"Marked"}).then(userData => {
          res.status(200).json({Data:userData})
          console.log(userData)
      })
...
exports.getbuinessdaybooking = async (req, res) => {
  try {
      ...
      await badge.find({ticketid:"636b00c990351e553fa1afa5"}).count().then(userData => {
          res.status(200).json({Data:userData})
          console.log(userData)
      })
...
exports.getnonbuinessdaybooking = async (req, res) => {
  try {
      ...
      await badge.find({ticketid:"636b010d90351e553fa1afa7"}).count().then(userData => {
          res.status(200).json({Data:userData})
          console.log(userData)
      })
